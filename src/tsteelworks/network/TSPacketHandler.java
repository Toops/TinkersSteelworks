...
public class TSPacketHandler implements IPacketHandler
{
	...
	public void onPacketData (INetworkManager manager, Packet250CustomPayload packet, Player player)
	{
		...
		if (packet.channel.equals(Repo.modChan)) if (side == Side.SERVER) handleServerPacket(packet, (EntityPlayerMP) player);
		else handleClientPacket(packet, (EntityPlayer) player);
	}
			...
			if (packetID == 1) // High Oven
			{
				...
				if (te instanceof HighOvenLogic)
				{
					...
					for (final FluidStack liquid : ((HighOvenLogic) te).moltenMetal)
						if (liquid.fluidID == fluidID) temp = liquid;
					if (temp != null)
					{
						((HighOvenLogic) te).moltenMetal.remove(temp);
						if (isShiftPressed) ((HighOvenLogic) te).moltenMetal.add(temp);
						else ((HighOvenLogic) te).moltenMetal.add(0, temp);
					}

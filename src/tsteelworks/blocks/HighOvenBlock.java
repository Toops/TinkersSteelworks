...
public class HighOvenBlock extends InventoryBlock
{
			...
			float clickZ)
	{
		...
		if ((integer == null) || (integer == -1)) return false;
		else
		{
	...
	public void onNeighborBlockChange (World world, int x, int y, int z, int nBlockID)
	{
		...
		if (logic instanceof IServantLogic) ((IServantLogic) logic).notifyMasterOfChange();
		else if (logic instanceof IMasterLogic) ((IMasterLogic) logic).notifyChange(null, x, y, z);
	}
	...
	{
		final String[] textureNames = { "scorchedbrick", "highoven_inactive", "highoven_active", "drain_out", "drain_basin" };
		if (!texturePrefix.equals("")) for (int i = 0; i < textureNames.length; i++)
	...
	public Icon getBlockTexture (IBlockAccess world, int x, int y, int z, int side)
	{
		...
		if (meta == 0)
		{
			if (side == direction)
			{
				if (isActive(world, x, y, z)) return icons[2];
				else return icons[1];
			}
			else return icons[0];
		}
		...
		{
			if (side == direction) return icons[4];
			else if ((side / 2) == (direction / 2)) return icons[3];
			else return icons[0];
		}
		else if (meta == 2) return icons[0];
		return icons[3 + meta];

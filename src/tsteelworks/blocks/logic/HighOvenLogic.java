...
public class HighOvenLogic extends InventoryLogic implements IActiveLogic, IFacingLogic, IFluidTank, IMasterLogic
{
	...
	void adjustLayers (int lay, boolean forceAdjust)
	{
		if ((lay != layers) || forceAdjust)
		{
			...
			if ((activeTemps.length > 0) && (activeTemps.length > tempActive.length)) for (int i = tempActive.length; i < activeTemps.length; i++)
			{
				activeTemps[i] = 20;
				meltingTemps[i] = 20;
			}
			if (tempInv.length > inventory.length) for (int i = inventory.length; i < tempInv.length; i++)
			{
				...
				if (stack != null)
				{
					...
					while (stack.stackSize > 0)
					{
						...
						if (itemSize > stack.stackSize) itemSize = stack.stackSize;
						stack.stackSize -= itemSize;
						final EntityItem entityitem = new EntityItem(worldObj, xCoord + jumpX + offsetX, yCoord + jumpY, zCoord
								+ jumpZ + offsetZ, new ItemStack(stack.itemID, itemSize, stack.getItemDamage()));
	...
	void heatItems ()
	{
		if (useTime > 0)
		{
			...
			for (int i = 0; i < layers; i++)
				if ((meltingTemps[i] > 20) && isStackInSlot(i))
				{
					hasUse = true;
					if ((activeTemps[i] < internalTemp) && (activeTemps[i] < meltingTemps[i])) activeTemps[i] += 1;
					else if (activeTemps[i] >= meltingTemps[i]) if (!worldObj.isRemote)
					{
						...
						{
							if (inventory[i].stackSize >= 2) inventory[i].stackSize--;
	...
	boolean addMoltenMetal (FluidStack liquid, boolean first)
	{
		...
		else
		{
			...
			if (!added) if (first) moltenMetal.add(0, liquid.copy());
	...
	void updateTemperatures ()
	{
		inUse = true;
		for (int i = 0; i < layers; i++)
			meltingTemps[i] = HighOvenSmelting.getLiquifyTemperature(inventory[i]);
	}
	...
	public void updateFuelDisplay ()
	{
		...
		final TileEntity tankContainer = worldObj.getBlockTileEntity(activeLavaTank.x, activeLavaTank.y, activeLavaTank.z);
		...
		if (tankContainer instanceof IFluidHandler)
		{
			...
			else
			{
				...
				while (!foundTank)
				{
					...
					{
						final FluidStack newliquid = ((IFluidHandler) newTankContainer).drain(ForgeDirection.UNKNOWN, 150, false);
					...
					iter++;
					if (iter >= lavaTanks.size()) foundTank = true;
				}
			}
		}
	}
	...
	{
		return HighOvenSmelting.getSmelteryResult(stack);
	}
	...
	public void checkValidStructure (int x, int y, int z)
	{
		...
		if ((structureCapped != tempValidStructure != validStructure) || (checkLayers != layers)) if (tempValidStructure
				&& structureCapped)
		{
			internalTemp = 800;
	...
	public boolean checkSameLevel (int x, int y, int z)
	{
		...
		if ((numBricks == 8) && (lavaTanks.size() > 0)) return true;
		else return false;
	}
	...
	public int recurseStructureUp (int x, int y, int z, int count)
	{
		...
		for (int xPos = x - 0; xPos <= (x + 0); xPos++)
			for (int zPos = z - 0; zPos <= (z + 0); zPos++)
			{
				...
				if ((block != null) && !block.isAirBlock(worldObj, xPos, y, zPos)) if (validBlockID(blockID)) return validateTop(
						x, y, z, count);
				else return count;
			}
		// Check outer layer
	...
	public int recurseStructureDown (int x, int y, int z, int count)
	{
		...
		for (int xPos = x - 0; xPos <= (x + 0); xPos++)
			for (int zPos = z - 0; zPos <= (z + 0); zPos++)
			{
				...
				if ((block != null) && !block.isAirBlock(worldObj, xPos, y, zPos)) if (validBlockID(blockID)) return validateBottom(
						x, y, z, count);
				else return count;
			}
		// Check outer layer X
	...
	public int validateTop (int x, int y, int z, int count)
	{
		...
		for (int xPos = x - 1; xPos <= (x + 1); xPos++)
			for (int zPos = z - 1; zPos <= (z + 1); zPos++)
				if (validBlockID(worldObj.getBlockId(xPos, y, zPos)) && (worldObj.getBlockMetadata(xPos, y, zPos) >= 1)) topBricks++;
		if (topBricks == 9) structureCapped = true;
		return count;
	}
	...
	public int validateBottom (int x, int y, int z, int count)
	{
		...
		for (int xPos = x - 1; xPos <= (x + 1); xPos++)
			for (int zPos = z - 1; zPos <= (z + 1); zPos++)
				if (validBlockID(worldObj.getBlockId(xPos, y, zPos)) && (worldObj.getBlockMetadata(xPos, y, zPos) >= 2)) bottomBricks++;
		if (bottomBricks == 9)
		{
	...
	int checkBricks (int x, int y, int z)
	{
		...
		if (validBlockID(blockID) || validTankID(blockID))
		{
			...
			if (te == this) tempBricks++;
			else if (te instanceof MultiServantLogic)
			{
				final MultiServantLogic servant = (MultiServantLogic) te;
				if (servant.hasValidMaster())
				{
					if (servant.verifyMaster(this, worldObj, xCoord, yCoord, zCoord)) tempBricks++;
				}
				else if (servant.setMaster(xCoord, yCoord, zCoord)) tempBricks++;
				if (te instanceof LavaTankLogic) lavaTanks.add(new CoordTuple(x, y, z));
			}
		}
		return tempBricks;
	}
	...
	public FluidStack drain (int maxDrain, boolean doDrain)
	{
		...
		else return new FluidStack(0, 0);
	}
	...
	public void readFromNBT (NBTTagCompound tags)
	{
		...
		if (center.length > 2) centerPos = new CoordTuple(center[0], center[1], center[2]);
		else centerPos = new CoordTuple(xCoord, yCoord, zCoord);
	...
	public void writeToNBT (NBTTagCompound tags)
	{
		...
		if (centerPos == null) center = new int[] { xCoord, yCoord, zCoord };
		else center = new int[] { centerPos.x, centerPos.y, centerPos.z };
	...
	public boolean isUseableByPlayer (EntityPlayer entityplayer)
	{
		...
		else return entityplayer.getDistance(xCoord + 0.5D, yCoord + 0.5D, zCoord + 0.5D) <= 64D;
	}
	...
	{
		if (slot < getSizeInventory()) if ((inventory[slot] == null)
				|| ((itemstack.stackSize + inventory[slot].stackSize) <= getInventoryStackLimit())) return true;
		return false;
	}
}

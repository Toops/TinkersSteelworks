...
public class HighOvenGui extends NewContainerGui
{
	...
	protected void drawGuiContainerForegroundLayer (int mouseX, int mouseY)
	{
		...
		for (final FluidStack liquid : logic.moltenMetal)
		{
			...
			if ((mouseX >= leftX) && (mouseX <= (leftX + sizeX)) && (mouseY >= topY) && (mouseY < (topY + sizeY))) drawFluidStackTooltip(
					liquid, (mouseX - cornerX) + 36, mouseY - cornerY);
		}
		if (logic.fuelGague > 0)
		{
			...
			if ((mouseX >= leftX) && (mouseX <= (leftX + sizeX)) && (mouseY >= topY) && (mouseY < (topY + sizeY))) drawFluidStackTooltip(
					new FluidStack(-37, logic.fuelAmount), (mouseX - cornerX) + 36, mouseY - cornerY);
		}
	}
	...
	public List getLiquidTooltip (FluidStack liquid, boolean par2)
	{
		...
		else
		{
			...
			else list.add("mB: " + liquid.amount);
		}
		return list;
	}

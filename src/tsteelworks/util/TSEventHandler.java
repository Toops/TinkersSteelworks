    ...
    public void bucketFill (FillBucketEvent evt)
    {
        if ((evt.current.getItem() == Item.bucketEmpty) && (evt.target.typeOfHit == EnumMovingObjectType.TILE))
        {
            ...
            if ((evt.entityPlayer != null) &&
                !evt.entityPlayer.canPlayerEdit(hitX, hitY, hitZ, evt.target.sideHit, evt.current)) return;
            final int bID = evt.world.getBlockId(hitX, hitY, hitZ);
            for (int id = 0; id < TSContent.fluidBlocks.length; id++)
                if (bID == TSContent.fluidBlocks[id].blockID) if (evt.entityPlayer.capabilities.isCreativeMode)
                {
                    evt.world.setBlockToAir(hitX, hitY, hitZ);
                }
                else
                {
                    if (TSContent.fluidBlocks[id] instanceof LiquidMetalFinite)
                    {
                        evt.world.setBlockToAir(hitX, hitY, hitZ);
                    }
                    else
